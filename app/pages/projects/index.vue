		...
		<div class="grid grid-flow-row sm:grid-cols-2 lg:grid-cols-3 gap-6">
			<ContentList v-slot="{ list }" :query="query">
				<article v-for="article in list" :key="article._path"
					...
					<NuxtLink v-if="article.image" :to="localePath(article._path!)" :aria-label="article.title">
...
const query: QueryBuilderParams = { path: '/projects', where: [{ _locale: locale.value }], limit: 10, sort: [{ _path: -1 }] }
